...
function initNeuralNetworkAnimation() {
  ...
  function updateConnections() {
    nodes.forEach((node, index) => {
      ...
      const line = connectionsSvg.querySelector(`.connection-${index}`);
      if (line) {
...
function handleNewsletterSignup(e) {
  ...
  if (!emailRegex.test(email)) {
    ...
    emailInput.style.boxShadow = "0 0 12px rgba(238, 108, 77, 0.3)";
    setTimeout(() => {
      emailInput.style.borderColor = "rgba(64, 224, 208, 0.2)";
  ...
  submitButton.disabled = true;
  submitButton.innerHTML = '<span class="submit-icon">⏳</span>';
  ...
  setTimeout(() => {
    ...
    emailInput.style.borderColor = "rgba(34, 197, 94, 0.3)";
    emailInput.style.background = "rgba(34, 197, 94, 0.05)";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Modal events
  primaryCTA.addEventListener("click", openModal);
  ...
  // Video demo
  if (playDemo) {
    playDemo.addEventListener("click", playVideoDemo);
  }
...
function initCTAEnhancements() {
  ...
  const ctaRevealObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
